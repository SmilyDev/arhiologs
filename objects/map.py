...
class Map(Object):
	...
	def draw(self, player):
		...
		for cell in range(self.mapSize[0]):
			pg.draw.line(self.screen, 'WHITE', [self.image.x + cell * self.cellSize, self.image.y], [self.image.x + cell * self.cellSize, self.image.y + self.mapSizePx[1]])
		#Горзонтальные линии
		for cell in range(self.mapSize[1]):
			pg.draw.line(self.screen, 'WHITE', [self.image.x, self.image.y + cell * self.cellSize], [self.image.x + self.mapSizePx[0], self.image.y + cell * self.cellSize])
